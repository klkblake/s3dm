...
func (m *Mat3) Right() *V3 {
	return NewV3(m.matrix[0], m.matrix[3], m.matrix[6])
}

func (m *Mat3) Up() *V3 {
	return NewV3(m.matrix[1], m.matrix[4], m.matrix[7])
}

func (m *Mat3) Forward() *V3 {
	return NewV3(m.matrix[2], m.matrix[5], m.matrix[8])
}

func (m *Mat3) SetRightUpForward(right, up, forward *V3) {
	m.matrix[0] = right.X; m.matrix[3] = right.Y; m.matrix[6] = right.Z
	m.matrix[1] = up.X; m.matrix[4] = up.Y; m.matrix[7] = up.Z
	m.matrix[2] = forward.X; m.matrix[5] = forward.Y; m.matrix[8] = forward.Z
}
